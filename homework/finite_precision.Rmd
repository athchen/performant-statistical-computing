...
<font color="blue"> The `determine_precision` function looks to see at what value two numbers are no longer distinguishable. This value is the **precision**. 

* To identify the precision, we start by an estimate of `1`. 
* If the numbers `1` and `1 + precision_est` are distinguishable,e.g. `1 != 1 + precision_est`, then the precision is halved. 
* This process continues until `R` no longer returns that `1 != 1 + precision_est` is true. At this point, the lower bound for the precision (e.g. the next value of precision that would return `FALSE` for `1 != 1 + precision_est`) and the upper bound (e.g. the last value of our precision estimate that returned `TRUE` for `1 != 1 + precision_est`) is returned. </font>
...
# Fill in ------
# First-order approximation for the expected number of duplicates when drawing uniformly with replacement. 
est_num_expected_duplicate(n_draw, n_unique_double)

# number of duplicates
set.seed(20210330)
unif_draws <- runif(n_draw) # Draw from a uniform distribution
sum(duplicated(unif_draws)) # Total number of duplicates
```

<font color="blue"> The expected number of duplicates when drawing uniformly with replacement is less than 1, but the observed number of duplicates is $`r sum(duplicated(unif_draws))`$. Based on the Hofert (2020), 

> Many RNGs produce (pseudo-)random integers in the form of a recursion and such integers
are then location-scale transformed to (0, 1) and represented as doubles to produce (pseudo-
)random numbers.

I suspect that the duplicates may result during the location-scale transformations which result in higher density of sampling particular values over others. 
</font>
...
<font color="blue"> Since the centered difference approximates the derivative up to an $O(\Delta x^2)$ error, I'd expect the slope of the plotted line on the log-log scale to be 2 (of -2 as $\Delta x$ is plotted from large to small values.) </font>
...
<font color="blue"> The output for stepsizes between $2^{-1}$ to $2^{-15}$ seems to agree with what I expected. But for step sizes smaller than $2^{-15}$, the relative errors increases as the step size decreases. I presume this is due to numerical stability problems for smaller step size values. </font> 

**Bonus question 1:** What is the slope in the "latter part" of your plotted line?

<font color="blue"> Interestingly, the average slope in the latter part of the plotted line seems to be 2. </font>
...
<font color="blue"> My strategy would be to start at a crude estimate of $f'(x)$, and half $\Delta x$ until the change in $f'(x)$ is below some numerical tolerance. From the code below, we see that the optimal value of $\Delta x$ is where the slope of the relative error starts to change. </font>

```{r}
determine_delta <- function(func, x, init_dx = 1, tol = 1e-32) {
  curr_dx <- init_dx
  divider <- 2 
  while (abs(approximate_deriv(func, x, curr_dx) -
             approximate_deriv(func, x, curr_dx/2)) >= tol) {
    curr_dx <- curr_dx / divider
  }
  return(curr_dx)
}

optimal_delta <- log2(determine_delta(log_logistic_func, x))
print(paste0(
  "The optimal value of delta x is 2^", 
  optimal_delta, "."
))
```
...
<font color="blue"> The log-likelihood is `-Inf` when $\beta = (1, 1, ..., )$. </font>
...
<font color="blue"> The convergence code returned 1, indicating that we had hit the max iteration limit and did not find the penalized MLE. </font>
...
print(paste0("The convergence code is: ", optim_result$convergence))
```
...
check_convergence <- function(optim_out){
  if(optim_out$convergence != 0){ warning("optim did not converge.")}
}

check_convergence(optim_result)
```
...
approximate_grad <- function(func, x, dx = 10^-6) {
  ...
  for (i in 1:length(x)) {
    dx_vec <- rep(0, length(x))
    dx_vec[i] <- dx
    numerical_grad[i] <- (func(x + dx_vec) - func(x - dx_vec))/(2*dx)
  }
...
set.seed(20210330)
test_data <- rbinom(50, 1, 0.25)
# bernoulli 
compute_logp <- function(x) {
  # Fill in
  sum(test_data*log(x) + (1 - test_data)*log(1 - x))
}
compute_grad <- function(x) {
  sum(test_data/x - (1 - test_data)/(1 - x))
}
...
compute_logit_grad <- function(
  ...
  # Fill in
  logit_prob <- design_mat %*% reg_coef
  loglik_grad <- t(n_success - n_trial/(1 + exp(-logit_prob))) %*% design_mat
...
<font color="blue"> Yes, it did :) </font>
...
```{r}
approx_directional_deriv <- function(funs, x, v, t = 1e-6) {
  (funs(x + t*v) - funs(x))/t
}
...
assert(assertion, {
  ...
  dderiv <- compute_internet_ad_grad(reg_coef) %*% v
  are_all_close(dderiv, approx_dderiv)
})
```

<font color="blue"> Since the second derivative is just the derivative of the first derivative, we can show that the second directional derivative ie equal to $v' H v$ where $v$ is a unit vector, and $H$ is the hessian matrix. We can then check the consistency of our Hessian using the second directional derivative (rather than coordinate-wise approximations). </font>
