...
output_C <- ar_precision_matvec(v, auto_corr)
output_R <- ar_precision_matvec_R(v, auto_corr) 

assert("Rcpp function should return the same output as the R function", {
  are_all_close(output_C, output_R)
})
```
...
<font color="blue"> For matrix-vector multiplication, the version implemented in Rcpp tends to be about 8- to 10-fold faster. </font> 

```{r}
# Compare performences of the R and Rcpp implementation
bench::mark(ar_precision_matvec_R(v, auto_corr))
bench::mark(ar_precision_matvec(v, auto_corr))
```
<font color="blue"> The Rcpp implementation that finds the top 10 principal components via Lanczos algorithm is significantly faster than the algorithm that explicitly calculates the eigen decomposition of $\boldsymbol{\Sigma}$. The Rcpp implementation is around 30 times faster than the explicit eigen decomposition approach. </font>
...
# Output, printed for convenience since `eigen` takes a while 
#    user  system elapsed 
#   4.245   0.545   5.095 
...
<font color="blue"> Interestingly, the symmetric banded matrix implementation perfoms slightly slower than `ar_precision_matvec`. I suspect that this is because `sym_banded_matvec` is designed to accept a general, more memory efficient storage approach for banded matrices. However, in the specific case of an AR1 process, the efficient storage still takes more memory than necessary as the dignoal and off-diagonal entries are are repeated. Since there are fewer required values to do the calculation with `ar_precision_matvec`, there are probably fewer cache misses. </font>

```{r}
banded_mat <- cbind(c(1, rep((1 + auto_corr^2), ar_length - 2), 1), 
                    c(rep(-auto_corr, ar_length -1), 0))
banded_mat <- banded_mat/(1 - auto_corr^2)

assert("The banded implementation returns the same output as the full matrix implementation.", {
  output_full <- ar_precision_matvec(v, auto_corr)
  output_banded <- sym_banded_matvec(v, banded_mat)
  are_all_close(output_full, output_banded)
})
...
assert("`row_col_dot_matmat` returns the same as the R implementation", {
  AB_R <- A %*% B
  AB_C <- row_col_dot_matmat(A, B)
  are_all_close(AB_R, AB_C)
})
```
...
<font color="blue"> The column-oriented implementation is faster. Because R stores arrays in column major order, the address of the elements are closer, and there are fewer cache misses. Consequently, less time is spent fetching data from higher level caches. </font>

```{r}
# Check the output of `col_oriented_matmat`
assert("`col_oriented_matmat` returns the same as the R implementation", {
  AB_R <- A %*% B
  AB_C <- col_oriented_matmat(A, B)
  are_all_close(AB_R, AB_C)
})
...
<font color="blue"> Interestingly, the R implementation is faster than the column-oriented implementation the row and column matrix multiplication in Rcpp. </font>
...
<font color="blue"> Indeed, `matmat_via_eigen` is faster than all other implementations benchmarked above. </font>

```{r}
# Check and benchmark
# Check the output of `col_oriented_matmat`
assert("`matmat_via_eigen` returns the same as the R implementation", {
  AB_R <- A %*% B
  AB_C <- matmat_via_eigen(A, B)
  are_all_close(AB_R, AB_C)
})
